...
export default function StickerToolbar({ onAdd }: Props) {
  ...
  function addLocation() {
    ...
    onAdd(createLocationSticker(locationValue.trim()))
    closePanel()
  }
      ...
      {panel === 'location' && (
        <div className="p-3 rounded-xl mt-1" style={{ background: 'var(--bg-input)' }}>
          ...
          <input
            ...
            placeholder="Ej: Almería, España"
            className="w-full text-sm outline-none rounded-lg px-3 py-2 mb-2"
            style={{ background: 'var(--bg-card)', color: 'var(--text)', border: '1px solid var(--border)' }}
          />
          <button onClick={addLocation} className="w-full text-sm py-2 rounded-lg" style={{ background: 'var(--text)', color: 'var(--bg)' }}>
            añadir
          </button>
        </div>
      )}
